...
const Hero = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const [imgResult, popResult, pkgResult] = await Promise.all([
  ...
  return (
    <div 
      ...
      style={{ backgroundImage: `url(${_.get(image, "[0].background_image")})` }} 
      ref={ripplesRef}
    >
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent z-10" />
      
      <div className="z-20 flex flex-col items-center gap-y-4 lg:py-24 py-12 px-4 w-full text-center">
        <h1 className="lg:text-6xl text-3xl font-extrabold text-white tracking-wide drop-shadow-lg">
          Waves of Wonder
        </h1>
        <p className="lg:text-2xl text-base text-white font-medium opacity-90">
          Book Your Scenic Ferry Adventure!
        </p>
        <Link 
          to="https://booking.sailsubham.com/home" 
          className="mt-4 font-bold text-white bg-gradient-to-r from-primary to-secondary hover:from-secondary hover:to-primary py-3 px-6 rounded-full shadow-lg flex items-center gap-x-2"
        >
                  ...
                  <div className="absolute w-full h-auto top-6 left-1/2 -translate-x-1/2 text-center px-5">
                    ...
                    {item.message_description && (
                      <div className="flex items-start sm:items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg shadow-sm border border-gray-200">
                        ...
                        <span className="text-sm sm:text-xs font-medium text-gray-800 leading-snug line-clamp-2">
                          {item.message_description}
                        </span>
                      </div>
                    )}
                    <div className="text-xl font-bold text-black flex items-center justify-center gap-3 mt-2">
                      ...
                      <div className="h-4 w-auto overflow-hidden shadow-sm">
                        <img 
                          src={item.country === "india" ? 
                            "https://cdn.britannica.com/13/4413-050-98188B5C/Flag-Sri-Lanka.jpg" : 
                            "https://img.freepik.com/premium-vector/flag-india-rectangle-icon-illustration-india-flag-rectangular-flag-standard-size_833685-3618.jpg?semt=ais_hybrid&w=740"} 
                          alt="country-flag" 
                          className="w-full h-full object-cover" 
                        />
                      </div>
      ...
      {selectedPackage && (
        <Modal 
          title={`Package Details: ${selectedPackage.package_address}`} 
